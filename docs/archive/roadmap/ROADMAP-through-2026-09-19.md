# Historical Roadmap Snapshot — through 2026-09-19

> Historical record — not authoritative for current implementation status or next work. Use [the active ROADMAP](../../ROADMAP.md).
>
> This snapshot preserves the former combined roadmap/delivery ledger. Machine-local Markdown links were normalized to repository-relative targets; historical prose and evidence are otherwise retained.
...
- UI Controls: [AdaptiveLayoutBadge.tsx](../../../app/components/dashboard/AdaptiveLayoutBadge.tsx) in the dashboard header displays live status (Off, Active, Paused, Deferred) with quick controls. [AdaptiveLayoutModal.tsx](../../../app/components/dashboard/AdaptiveLayoutModal.tsx) allows inspecting rules, toggling individual rules, and restoring prior snapshots.
...
- **15-Point Acceptance Matrix**: Completely verified in [dashboard-acceptance-gate.test.ts](../../../tests/dashboard-acceptance-gate.test.ts) across all 3 personas (PMHNP, Practice Owner, Practice Manager & Biller).
- **Controlled Reversible Default Switch**: Primary landing view defaults to `TodayDashboard` (`defaultLandingView: "today"`) upon login, with complete user-controlled reversibility via [WorkspaceCustomizer.tsx](../../../app/components/WorkspaceCustomizer.tsx), runtime preference persistence, and environment override `NEXT_PUBLIC_DEFAULT_WORKSPACE_VIEW`. Zen Home remains a 1-click companion launcher/pad preserving single-architecture integrity.
...
- **Architectural Decision**: Formally documented in [D-058](../../../docs/DECISIONS.md).
