...
- **NAV-04 / WIN-09; SAVE-08 partial (2026-09-09):** Non-Encounter chart scrolling is keyed by explicit patient ID and section, with the same content identity across docking/redocking and independent horizontal floating section-tab scrolling. Scroll events capture positions synchronously so a deferred callback cannot read a reused patient surface. If a chart surface mounts before async/late content is large enough to reach the saved coordinate, the scroll manager now retains the original target and advances restoration as later content mutations make that coordinate reachable instead of overwriting it with the temporary clamped position; deliberate clinician scrolling cancels that pending restore. Unit coverage verifies identity isolation, docking equivalence, missing-identity rejection, and bounded progressive restoration. Old display-text-based positions are intentionally not migrated. Storage remains browser-session-only, not clinician-scoped durable restoration; Encounter nested scrolling and visual lifecycle verification remain outstanding.
