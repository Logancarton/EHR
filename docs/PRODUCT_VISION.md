...
- **NAV-04 / WIN-09; SAVE-08 partial (2026-09-09):** The existing scroll manager now keys non-Encounter chart content by explicit patient ID and section, sharing the same identity across docking/redocking. Horizontal floating section-tab scrolling remains independent. Scroll events capture positions synchronously so a deferred callback cannot read a reused patient surface. Unit coverage verifies identity isolation, docking equivalence, and missing-identity rejection. Old display-text-based positions are intentionally not migrated. Storage remains browser-session-only, not clinician-scoped durable restoration; Encounter nested scrolling, late-loading content, and visual lifecycle verification remain outstanding.
