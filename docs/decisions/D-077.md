...
Implementation status: Implemented: prospect-stage documents and coverage remain the same rows through promotion.
...
Documentation date corrected 2026-09-19 against implementation commit `d129bf5` (2026-09-17, America/Phoenix). The migration identity `2026-09-18-001-document-insurance-prospective-identity` is unchanged.Decision: D-076 deliberately left `documents` and `insurance_policies` chart-only,
