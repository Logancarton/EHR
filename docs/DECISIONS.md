...
| [D-103](decisions/D-103.md) | Active / Accepted | Implemented in STORE-1: vitals are entered only through the vitals form, and `add_observation` refuses vital signs and unknown categories with strict API validation; the Labs section gains a Record result form; seeded office BPs are vitals (migration converts old rows); the chart snapshot's observations exclude vitals so vitals cannot push lab history out; care-completion reads `laboratory`; scratch notes move to an author- and practice-scoped `scratch_notes` table. | Vitals and lab results have separate entry and storage; scratch notes belong to their author |
| [D-104](decisions/D-104.md) | Active / Accepted | Implemented in NOTE-SAFE-1: a new note starts with a blank MSE and chief complaint, and a normal exam is inserted only by an explicit click. Unsigned drafts lose the old unauthored default, on load and by migration `2026-09-25-003`. Safety is met only by a written Risk Assessment. The Overview and the note share one "latest administration per instrument" rule for current safety flags, and a current flag leads the note. Drafts no longer default to 99214. A 404 for a not-yet-saved draft is not a failed read. Amends D-100 and D-101. | A new note assumes no findings, and safety is grounded in what the clinician wrote |
