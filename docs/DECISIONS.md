...
| [D-085](decisions/D-085.md) | Active — amended | Accepted target; implementation begins with the universal `+` launcher, then migrates Team/Communication through the companion expand/redock parity gate before old navigation is removed. D-086 amends only its Staff/HR placement, D-087 only its Settings placement, and D-089 only Tasks' ownership and what a companion owes a capability it owns. | Home + workspace tabs + universal `+` launcher with expandable contextual canvases |
