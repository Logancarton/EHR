...
- **Not done / named gaps.** Workflow has not replaced either Charges table; that is a later step once parity is agreed. On short viewports, the shared "… recorded." confirmation box takes a lot of height. The stage filter and sort are not persisted across reloads. Coding fixes point to the chart, but a signed note is immutable, so resolving a missing diagnosis on an already-signed encounter still has no in-product path beyond voiding.
