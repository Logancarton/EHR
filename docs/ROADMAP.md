...
Latest implementation slice: UI-8, which gave Dashboard the `+` launcher entry it lacked and then removed the top-bar work-navigation row, completing the owner-directed shell migration ([D-094](decisions/D-094.md)). It is verified per spec rather than by a full-suite run — read its Evidence before quoting a suite figure. The last slice validated by a full run is UI-7d, which moved Prescribing to the right companion and removed Clinical with it ([D-092](decisions/D-092.md)); before it, CB-0b attributed every standing browser failure to a named cause and repaired all but one ([D-091](decisions/D-091.md)).
...
**The next eligible slice is CB-7**, the clinical certification gate, unless the owner reprioritises. The owner's 2026-09-22 report named three bottlenecks beyond navigation redundancy that UI-8 does **not** address; each is its own slice with its own evidence, and none of them is a chrome removal. The nearest of them is the overlap between the right icon rail and the `+` launcher: the rail's Calendar, Chat, Tasks and Settings are companion tools in the [D-085](decisions/D-085.md) lifecycle rather than workspace destinations, and whether that distinction is legible to a clinician is a real question this slice deliberately did not answer.
