...
| **UI-6** | Decompose **Practice** one child at a time: Billing -> major workspace; Website + Social Media -> Brand; Staff/HR -> companion canvas with expand; Settings -> profile/preferences; Reports -> assigned to its owning workspace when real. | **In progress.** Billing (UI-6a) and Website + Social media (UI-6b/UI-6c) rehomed and removed from the menu; Reports was already filtered out as `planned`. Staff/HR and Practice settings still have no verified owner, so Practice stands. |
...
| Staff directory (`hr`) | Companion canvas with expand (D-085) — not built | Still in Practice |
...
- **Remaining before UI-6 can close:** Staff/HR needs the companion canvas with expand that D-085 assigns it; Practice settings needs a verified profile/preferences owner; Reports stays parked until it is real. Practice is removed only when nothing depends on it.
- **Diagnose before moving the last two children.** Practice's two remaining entries both land on staff-shaped screens, and they overlap. `settings` renders `PracticeStaffWorkspace`, a "People" surface owning accounts, roles, sign-in access and the practice's default layouts — part preferences, part staff administration. `hr` renders `HRStaffWorkspace`, a staff directory and provider-credentialing surface. Deciding what is preferences, what is staff administration, and what is credentialing has to come before either child is rehomed, or the split will be made by whichever menu item is deleted first.
