...
11. **Remaining production infrastructure for Phase 2.** (**Active Next**: password reset for a forgotten password, source-address rate limiting behind a trusted proxy, SSO and device inventory; production database path and migration/deployment; encryption/secrets deployment; protected object storage; audit retention/access review; backup restoration; monitoring/incident response).
