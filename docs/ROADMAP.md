...
Reviewed: 2026-09-09 against GitHub `main` through `b7511f4`; updated by the floating-window gesture ownership slice, the clinical-facts absence-assertion slice, and an inspection of verification tooling and the authenticated API surface in this change.
...
| Clinical record lifecycles — implemented foundations | Normalized medication/problem/allergy records, lab-result lifecycle, provenance/version history, immutable signed encounters, charted communications and document review/filing. Clinical-fact presentation distinguishes not-yet-loaded and failed-to-load from a loaded absence, and is derived against the active patient so another patient's facts are never shown under their identity. [Architecture](ARCHITECTURE.md), [documents](document-workflow.md), [message charting](message-charting.md), [facts presentation](../app/lib/clinical-facts-presentation.ts), [D-030](DECISIONS.md). | End-to-end workflow verification; durable document binary upload/retrieval and production object storage; import/reconciliation and related-evidence workflows. Existing document metadata/version workflows must be reused. |
...
2. **Prerequisite, not yet satisfied:** select a browser verification harness and record the choice in [DECISIONS.md](DECISIONS.md). No browser/DOM test tooling exists in the repository as of `b7511f4` — no Playwright, Puppeteer, Cypress, jsdom or Testing Library in `package.json` or `node_modules` — and all 38 test files run as Node unit tests under `tsx --test`. Until a harness is chosen, steps 3–5 and this phase's exit gate cannot be satisfied by any test the project is able to run. Weigh it as a durable dependency decision: real pointer-capture and drag semantics for the eight resize directions, deterministic control of late async responses, and reasonable cost on Windows. Do not add a harness incidentally as part of a feature slice.
...
Exit gate: affected browser scenarios pass with two synthetic patients and an unfinished draft, including canceled gestures and late responses. Passing unit tests alone does not close this phase, and no tooling capable of closing it is currently installed; step 2 gates the rest.
...
- **First, close the endpoints that are already open.** Bring `GET /api/patients` and `GET /api/audit` behind the existing authentication and permission helpers, then sweep every route under `app/api/` for handlers that reference no actor. Audit-log readability is the more serious of the two, since audit integrity is an architectural requirement rather than a later control. Add a regression test asserting that an unauthenticated request to a clinical or audit route is rejected. This is remediation of a present defect and does not depend on any production infrastructure decision.
...
1. Select and record the browser verification harness (Phase 0 step 2). Nothing else in this queue can be verified to its stated gate without it.
2. Close the unauthenticated `GET /api/patients` and `GET /api/audit` handlers and add the access-denial regression test (Phase 2). Small, self-contained, and independent of the harness decision, so it need not wait behind it.
3. Repeatable two-patient browser lifecycle coverage, then fix the failures it demonstrates. Exercise detach, movement, all eight resize directions, protected controls, dock by button/drag, snap preview/placement, minimize/restore, maximize/restore, title-bar double-click, focus/z-order, multiple visible patient charts, unfinished encounter draft preservation, canceled gestures and late async responses.
4. Finish the next evidence-handling slice: durable document content intake/retrieval through the existing document workflow. The document schema currently stores workflow metadata only — no content column, storage reference or upload path exists.
5. Verify the complete synthetic visit and queue-to-object-to-action flow with patient-bound late-response handling.
