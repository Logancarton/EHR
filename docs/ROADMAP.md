...
| Identity and persistence — implemented development foundation | Server sessions, authoritative roles, strict direct-read helper, patient-bound action gateway, permission-aware context, SQLite records, audit, clinician-owned preferences, migration ledger and secret-reference/readiness boundaries. Organization membership and patient-access scope are enforced independently of authentication and roles (D-033), and administered through an audited endpoint confined to the administrator's own practice, with session revocation on revocation/deactivation (D-038). | Production login provisioning (password/SSO) and device controls; production database path; encryption/secrets deployment; backups/restore; monitoring/incident response. |
...
Remaining:
0. ~~User provisioning and organization administration.~~ **Completed** in D-038: audited provisioning, membership status/scope changes, and activation, each confined to the acting administrator's own organization, with live sessions revoked on revocation or deactivation.
...
Status: **Internal prescribing architecture implemented; deferred by owner decision (D-037).** Requirements: RX-01–05.

Logan is not obtaining DrFirst access at this time, so this phase is not being worked. DrFirst remains the recorded selection should that change (D-028). Establish the actual contracted product/interface, sandbox access, credentials and onboarding/testing requirements before representing connectivity as functional. Then map one adapter edge into the existing transaction/callback/recovery architecture and verify ambiguous sends, replay, cancellation, renewal/change requests and EPCS through the supported vendor workflow. Vendor evidence never automatically changes medication truth.
...
7. **Prepare DrFirst integration from verified vendor requirements.** (**Deferred by owner decision, D-037.** Logan is not obtaining DrFirst access at this time. Do not prepare, map, or stub a vendor interface until that changes; the development adapter continues to fail closed).
8. **User provisioning and organization administration.** (Completed: `manage_organization`, `OrganizationAdminService`, `GET/POST/PATCH /api/organization/members`, session revocation on membership revocation and deactivation, and `tests/organization-administration.test.ts`. See D-038).
9. **Account credential lifecycle.** (Completed: administrator-issued single-use activation tokens, holder-chosen passwords at `POST /api/auth/activate`, self-service password change at `POST /api/auth/password` with session revocation, and `tests/auth-credential-lifecycle.test.ts`. See D-039).
10. **Login rate limiting.** (Completed: durable per-username failed-attempt counters, lockout ahead of the password check, uniform error messages, audited lock/unlock, and administrator clearing. See D-040).
11. **Production database path and proven recovery.** (Completed: `EHR_DATABASE_PATH` required and absolute in production, online backup with immediate verification, verified restore that displaces rather than deletes, `scripts/database-backup.ts`, and `tests/database-recovery.test.ts`. See D-041).
12. **Remaining production infrastructure for Phase 2.** (**Active Next**: encryption at rest and production secrets deployment; protected object storage for documents; audit retention and access review; backup scheduling, offsite storage and restore rehearsal; monitoring and incident response; password reset for a forgotten password; source-address rate limiting behind a trusted proxy; SSO and device inventory).
