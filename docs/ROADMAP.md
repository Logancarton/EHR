...
- **Remaining before UI-6 can close:** Practice settings needs a verified profile/preferences owner; Reports stays parked until it is real. Practice is removed only when nothing depends on it.
- **Diagnose before moving Practice settings.** `settings` renders `PracticeStaffWorkspace`, a "People" surface owning accounts, roles, sign-in access and the practice's default layouts — part preferences, part staff administration. Now that HR (D-086) owns employee records with its own authorization boundary, the overlap needs deciding: what belongs to preferences, what to organization administration, and what to HR. Making that call by deleting a menu item is how the three quietly become two.
