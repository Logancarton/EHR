...
export type ClinicalAction =
  ...
  | { type: "add_medication"; payload: { patientId: string; displayText: string; medicationName?: string; genericName?: string; strength?: string; dose?: string; route?: string; frequency?: string; indication?: string; startDate?: string; prescriber?: string; source?: RecordSource } }
  | { type: "update_medication"; payload: { recordId: string; patch: { displayText?: string; medicationName?: string; genericName?: string | null; strength?: string | null; dose?: string | null; route?: string | null; frequency?: string | null; indication?: string | null; startDate?: string | null; prescriber?: string | null; status?: "active" | "discontinued" | "completed" | "entered-in-error"; endDate?: string | null }; source?: RecordSource } }
  ...
  | { type: "add_insurance"; payload: { patientId: string; payerName: string; planName?: string; memberId?: string; groupNumber?: string; subscriberName?: string; subscriberDob?: string; relationship?: string; effectiveDate?: string; coverageType?: string; coveragePriority?: number; isSelfPay?: boolean; source?: RecordSource } }
  | { type: "update_insurance"; payload: { recordId: string; patch: { payerName?: string; planName?: string | null; memberId?: string | null; groupNumber?: string | null; subscriberName?: string | null; subscriberDob?: string | null; relationship?: string | null; coverageType?: string; coveragePriority?: number; isSelfPay?: boolean; status?: "active" | "inactive" | "terminated" | "entered-in-error"; terminationDate?: string | null }; source?: RecordSource } }
  ...
  | { type: "check_in_appointment"; payload: { appointmentId: string; expectedVersion?: number } }
  | { type: "start_visit_appointment"; payload: { appointmentId: string; expectedVersion?: number } }
  | { type: "complete_appointment"; payload: { appointmentId: string; expectedVersion?: number } }
  | { type: "mark_no_show_appointment"; payload: { appointmentId: string; expectedVersion?: number } }
  | { type: "schedule_follow_up"; payload: { originAppointmentId: string; interval: string; date?: string; time?: string; type?: VisitType; duration?: string; providerId?: string; room?: string } }
...
export async function executeClinicalAction(envelope: ClinicalActionEnvelope) {
  ...
  switch (action.type) {
    ...
    case "complete_appointment": return workflowService.completeAppointment(action.payload.appointmentId, actor, context, action.payload.expectedVersion);
    case "mark_no_show_appointment": return workflowService.markNoShowAppointment(action.payload.appointmentId, actor, context, action.payload.expectedVersion);
    case "schedule_follow_up": return workflowService.scheduleFollowUpAppointment(action.payload, actor, context);
