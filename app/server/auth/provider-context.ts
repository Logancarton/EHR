...
type ProviderSession = ProviderContext & {
  issuedAt: number;
  expiresAt: number;
};
...
/**
 * Session issuer for a future OAuth/SSO callback. This signs an already-authenticated
 * identity; it does not authenticate usernames/passwords itself.
 */
export function createProviderSessionToken(
  ...
  const secret = sessionSecret();
  if (!secret) {
    throw new Error("EHR_SESSION_SECRET must be configured before issuing sessions.");
  }
...
/**
 * Resolve identity from a signed, server-issued session cookie.
 * Production fails closed when the session is absent/invalid. Local development keeps
 * a prototype identity so synthetic workflows remain usable before SSO is connected.
 */
